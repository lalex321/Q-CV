...
def sanitize_json(data):
    ...
    if clean_title:
            ...
            if _paren_match:
                ...
                # Remove noise phrases, keep tech tokens
                _paren_text = re.sub(r'\b\d+x\s+', '', _paren_text)  # "3x AWS" → "AWS"
...
def smart_anonymize_data(data, api_key, cfg):
    ...
    if cfg.get("anon_mask_companies", True):
        ...
        if all_companies:
            ...
            # Collect client/project names mentioned in brackets in project descriptions
            bracket_names = set()
            ...
            # Scrub company/client names from all text fields
